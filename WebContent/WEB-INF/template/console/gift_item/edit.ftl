...
<title>${message("console.giftItem.edit")} - 爱柚米 • 移动营销平台</title>
   ...
   <div id="wrapper">
	    ...
	    <div id="page-wrapper" class="gray-bg dashbard-1">
	        ...
	        <div class="row wrapper border-bottom white-bg page-heading">
				<div class="col-lg-10">
					<h2> ${message("console.giftItem.edit")} </h2>
	                <ol class="breadcrumb">
						<li>
						    <!--首页-->
	                    ...
	                    <li>
	                        <strong>${message("console.giftItem.edit")}</strong>
	                    </li>
					</ol>
	            </div>
	            <div class="col-lg-2">
			...
			<div class="row border-bottom white-bg-bak dashboard-header mainContent">
				<div class="panel blank-panel">
					...
					<form id="inputForm" action="update.ct" method="post" enctype="multipart/form-data">
						<input type="hidden" id="giftItemId" value="${giftItem.id}" name="id"/>
								...
								<div id="settingBaseTab" class="tab-pane active">
									<table class="table table-striped">
										<tr>
											...
											<td>
												<input type="text" name="giftName" class="form-control" value="${giftItem.giftName}"/>
											</td>
										</tr>
										<tr>
												...
												<div class="col-md-12">
													<div class="input-group" style="width:100%;">
				                                        <select id="schoolSelect" data-placeholder="请选择关联学校(可搜索名字)" name="dictSchoolIds" class="chosen-select" multiple style="width:100%;"  tabindex="4">
				                                            ...
				                                            <option value="${dictSchool.id}" hassubinfo="true" 
				                                            [#list relatedSchools as relatedSchool]
				                                            	[#if relatedSchool.id == dictSchool.id ]
					                                            	selected="true"
					                                            [/#if]
				                                            [/#list]
				                                            >${dictSchool.name}</option>
				                                            [/#list]
				                                        </select>
				                                    </div>
												</div>
										...
										<tr>
											...
											<td>
												<div class="col-sm-9">
													...
													<img style="display: none;" height="150px" id="showColumnCoverImage" src="${giftItem.image}"/>
													...
													<input type="hidden" id="columnCoverImage" name="image" class="form-control" value="${giftItem.image}"/>
										...
										<tr>
											...
											<td>
												<input type="text" id="price" name="price" class="form-control" value="${giftItem.price}"/>
											</td>
										</tr>
										<tr>
											<th>
											...
											<td>
												<input type="text" name="point" class="form-control" value="${giftItem.point}"/>
											</td>
										</tr>
										<tr>
											<th>
											...
											<td>
												<input type="checkbox" name="isOnline" id="isOnline" [#if giftItem.isOnline]checked="checked"[/#if]/>
										...
										<tr>
											...
											<td>
												<textarea name="decription" class="form-control" style="height: 80px;">${giftItem.decription}</textarea>
											</td>
										</tr>
										<tr>
											<th>
											...
											<td>
												<input type="text" name="quantity" class="form-control" value="${giftItem.quantity}"/>
											</td>
										</tr>
										<tr>
											<th>
											...
											<td>
												<input type="text" name="maxExchangeNum" class="form-control" value="${giftItem.maxExchangeNum}"/>
											</td>
										</tr>
										<tr>
											<th>
											...
											<td>
												<input type="checkbox" name="isBuy" id="isBuy" [#if giftItem.isBuy]checked="checked" value="true"[#else]value="false"[/#if]/>
												<lable for="isBuy">${message("GiftItem.buy")}</lable>
											</td>
										</tr>
										<tr id="defaultPriceChangeDiv" [#if !giftItem.isBuy]style="display:none;"[/#if]>
											<th>
												<span class="requiredField">*</span>${message("GiftItem.defaultPriceChange")}:
											</th>
											<td>
												<input type="text" id="defaultPriceChange" [#if giftItem.isBuy]name="defaultPriceChange"[/#if] class="form-control" value="${giftItem.defaultPriceChange}"/>
											</td>
										</tr>
									</table>
								</div>
								<div id="settingGiftItemRulesTab" class="tab-pane">
                                    <table class="table table-striped">
										...
										<table class="table table-striped" id="rulesTable">
											[#if giftRules ?? && giftRules?size > 0]
											[#list giftRules as giftRule]
											<tr>
												<th>${message("规则")}${giftRule_index+1}：</th>
												<td>
				   									<input type="text" class="form-control" name="contents" maxlength="200" value="${giftRule.content}"/>
												</td>
			   								</tr>
			   								[/#list]     
											[/#if]
